...
namespace Personality.Test.Controllers
{
    public class SelectionsControllerTests
    {
        ...
        {
            var controller = new SelectionsController(context);
            var answerIds = new List<long>() { 1, 5, 9, 13, 17 };
            var session = controller.AddSelections(new SelectionsDto() { AnswerIds = answerIds });
            
            var result = controller.GetSelections(session.Id);
        ...
        public void CreateNewSessionWithSelectionsTest()
        {
            ...
            var result = controller.AddSelections(new SelectionsDto() { AnswerIds = answerIds });
        ...
        public void TooLittleAnswersProvidedValidationTest()
        {
            ...
            var result = Assert.Throws<ValidationException>(() => controller.AddSelections(new SelectionsDto() { AnswerIds = answerIds }));
